"""
CARE-E2E-Fusion v2b 训练脚本 —— 降低蒸馏权重对照版
====================================================
改动（相对 v2）:
  1. distill_weight 默认 0.5 -> 0.1
  2. use_distillation 默认 True -> False（先跑无蒸馏基线）
  3. 其余超参与 v2 保持一致：L4, K16, label_smoothing=0.05, warmup=3
"""
...
class FusionModuleV2(nn.Module):
    def __init__(self, embed_dim=768, num_heads=8, num_layers=4,
                 ...
                 use_two_branches=True, distill_weight=0.1,
    ...
    def forward(self, tile_tokens, coords, labels=None,
        ...
        if self.use_two_branches:
            ...
            if labels is not None:
                ...
                gate = self.gate(x_global, region_pooled)
...
def get_args():
    ...
    p.add_argument('--exp_code', type=str, default='real_v2b_K16_L4_Distill01')
    p.add_argument('--embed_dim', type=int, default=768)
    p.add_argument('--num_heads', type=int, default=8)
    p.add_argument('--num_layers', type=int, default=4)
    p.add_argument('--num_region_tokens', type=int, default=16)
    p.add_argument('--num_tasks', type=int, default=1)
    p.add_argument('--lr', type=float, default=3e-4)
    p.add_argument('--warmup_epochs', type=int, default=3)
    p.add_argument('--reg', type=float, default=2e-4)
    p.add_argument('--batch_size', type=int, default=2)
    p.add_argument('--max_epochs', type=int, default=40)
    p.add_argument('--dropout', type=float, default=0.35)
    p.add_argument('--patience', type=int, default=12)
    p.add_argument('--stop_epoch', type=int, default=10)
    p.add_argument('--use_distillation', type=bool, default=False)  # v2b: 默认关闭
    p.add_argument('--distill_weight', type=float, default=0.1)     # v2b: 降低权重
