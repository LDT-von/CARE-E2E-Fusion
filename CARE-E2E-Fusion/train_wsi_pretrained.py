# -*- coding: utf-8 -*-
"""
E2E WSI Training with Pre-trained ViT Backbone
==========================================
Strategy:
1. Frozen pretrained ViT-B/16 from timm as tile encoder (86M params)
2. Train DRP + ARM + heads (~18M params)
3. Much faster convergence than training from scratch
"""
...
class WSIDatasetPretrained(Dataset):
...
# ============================================================
# Model: Frozen ViT + Trainable DRP + ARM + Heads
# ============================================================

class FusionModule(nn.Module):
    """Trainable fusion: DRP + ARM + heads, using ViT CLS tokens as tile features."""
    ...
    def __init__(self, embed_dim=768, num_heads=8, num_layers=4,
        ...
        # Transformer blocks (for processing tile tokens)
    ...
    def forward(self, tile_tokens, coords, labels=None, label_smoothing=0.1):
        """tile_tokens: [B, N, C]; coords: [B, N, 2]"""
...
def smoothed_bce(logits, labels, smoothing=0.1):
    if smoothing <= 0:
        return F.binary_cross_entropy_with_logits(logits, labels)
    smooth = labels * (1 - smoothing) + 0.5 * smoothing
...
def main():
    ...
    parser.add_argument('--results_dir', type=str, default='./results_wsi_pretrained')
    ...
    exp_code = 'E2E_WSI_Pretrained_K%d_L%d_s%d' % (
    ...
    print('ViT embed_dim:', embed_dim)
    total_params = sum(p.numel() for p in vit.parameters())
    print('ViT params (frozen): %d' % total_params)
        ...
        for epoch in range(args.max_epochs):
            ...
            for batch in pbar:
                ...
                with autocast(device_type='cuda'):
                    ...
                    with torch.no_grad():
                        feat = vit.forward_features(images_flat)
                        if isinstance(feat, tuple):
                            feat = feat[0]
                        tile_tokens = feat[:, 0]  # CLS token [B*N, 768]
                        tile_tokens = tile_tokens.view(B, N, -1)  # [B, N, 768]
